...
from watertap.unit_models.unified_reverse_osmosis_1D import (
    UnifiedReverseOsmosis1D,
    ROFlowPattern
)
...
def build():
    ...
    m.fs.unit = UnifiedReverseOsmosis1D(
        property_package=m.fs.properties,
        has_pressure_change=True,
        concentration_polarization_type=ConcentrationPolarizationType.fixed,
        mass_transfer_coefficient=MassTransferCoefficient.none,
        has_full_reporting=True,
        flow_pattern=ROFlowPattern.countercurrent,
        make_permeate_inlet_port=True
    )
...
@pytest.mark.skip(reason="structurally singular")
def build_SKK_CP_calculation_with_kf_fixed():
    ...
    Additionally, feed and permeate-side mass transfer coefficients are fixed.
    """
    m = ConcreteModel()
    m.fs = FlowsheetBlock(dynamic=False)
