...
class ROFlowPattern(StrEnum):
    """
    Enum of supported flow patterns.
    """
...
class UnifiedReverseOsmosisBaseData(InitializationMixin, UnitModelBlockData):
    """
    ...
    @staticmethod
    def _add_base_config_entries(config_obj):
        config_obj.declare(
            "flow_pattern",
            ConfigValue(default=ROFlowPattern.cocurrent, domain=In(ROFlowPattern)),
        )
        config_obj.declare(
            "make_permeate_inlet_port", ConfigValue(default=False, domain=Bool)
        )
        config_obj.declare(
            "has_full_reporting",
            ConfigValue(
    ...
    def build(self):
        ...
        if self.config.make_permeate_inlet_port:
            self.add_inlet_port(name="permeate_inlet", block=self.permeate_side)
        else:
            self._setup_permeate_boundary_condition()
        ...
        if not self.config.dynamic:
            self._add_steady_state_performance_metrics()
    ...
    def _add_flux_balance(self):
        ...
        if (
            ...
            def flux_mass_phase(b, t, x, p):
                return sum(
                    b.flux_mass_phase_comp[t, x, p2, j]
                    for (p2, j) in self.config.property_package.phase_component_set
                    if p2 == p
                )

        return self.eq_flux_mass

    def _add_steady_state_performance_metrics(self):
        # ==========================================================================
    ...
    def _setup_permeate_boundary_condition(self):
        raise NotImplementedError(
            "The permeate boundary condition should be implemented "
            "by a class inheriting from the unified RO base."
        )
